...
class Grafo {								//definicion de la clase
...
Grafo::Grafo() {							//constructor por defecto (sin par�metros)
...
void Grafo::setMaxVerts(int n){				//actualiza cantidad maxima de vertices
	maxVerts = n;
}

void Grafo::setNumVerts(int n){		 		//actualiza numero de vertices
	numVerts = n;
}

void Grafo::setVertice(int va, Vertice v) {			//modifica los atributos de un vertice, conociendo su numero
	if ((va >= 0) && (va < getNumVerts())) {				//numero de vertice si existe en el grafo
		verts[va] = v;										//actualiza atributos
	} 
}

void Grafo::setVertice(string a, Vertice v) {			//modifica los atributos de un vertice, conociendo su numero
...
int Grafo::getMaxVerts(){						//devuelve cantidad maxima de vertices
	return maxVerts;
}

int Grafo::getNumVerts(){						//devuelve cantidad de vertices
	return numVerts;
}

Vertice Grafo::getVertice(int va) {					//retorna todos los atributos del v�rtice, si existe en el grafo, conociendo su numero
...
Vertice Grafo::getVertice(string a) {					//retorna todos los atributos del v�rtice si est� almacenado en el grafo, conociendo su nombre
	int va;													//variable para contener numero de vertice
	va = getNumVertice(a);										//calcula numero de vertice del primer nombre recibido
...
int Grafo::getNumVertice(string v) {					//determina el numero de un vertice dentro del grafo, conociendo su nombre
...
void Grafo::nuevoVertice(string v) {					//crea un nuevo vertice recibiendo su nombre
...
void GrafoLista::setArco(string a, string b){				//actualiza valor de arco recibiendo nombres de vertices en grafos no valorados
	int va, vb;												//variables para contener numeros de los vertices recibidos por nombre
	va = getNumVertice(a);									//calcula numero de vertice del primer nombre recibido
	vb = getNumVertice(b);									//calcula numero de vertice del segundo nombre recibido
	if ((va >= 0) && (vb >= 0)) {							//comprueba existencia de los dos vertices
...
void GrafoLista::setArco(string a, string b, int v) {		//modifica el valor almacenado en lista de adyacencia, de dos vertices recibidos por su nombre
	int va, vb;												//variables para contener numeros de los vertices recibidos por nombre
	va = getNumVertice(a);									//calcula numero de vertice del primer nombre recibido
	vb = getNumVertice(b);									//calcula numero de vertice del segundo nombre recibido
	if ((va >= 0) && (vb >= 0)) {							//comprueba existencia de los dos vertices
...
int GrafoLista::getArco(int va, int vb) {					//retorna el valor del arco almacenado en la matriz de adyacencia para dos numeros de v�rtices que recibe como par�metro por numero
	if ((va >= 0) && (vb >= 0) && (va < getNumVerts()) && (vb < getNumVerts())){	//comprueba existencia de los dos vertices
		return arcos[va] -> buscarValorEnLista(vb) != NULL;
	}
}

int GrafoLista::getArco(string a, string b) {				//retorna el valor del arco almacenado en la matriz de adyacencia para dos v�rtices que recibe como par�metro por nombre
	int va, vb;												//variables para contener numeros de los vertices recibidos por nombre
	va = getNumVertice(a);									//calcula numero de vertice correspondiente al primer nombre recibido
	vb = getNumVertice(b);									//calcula numero de vertice correspondiente al segundo nombre recibido
	if ((va >= 0) && (vb >= 0)) {							//comprueba existencia de los dos vertices
...
bool GrafoLista::adyacente(string a, string b) {			//determina si dos vertices forman un arco conociendo su nombre
	int va, vb;												//variables para contener numeros de los vertices recibidos por nombre
	va = getNumVertice(a);									//calcula numero de vertice del primer nombre recibido
	vb = getNumVertice(b);									//calcula numero de vertice del segundo nombre recibido
