...
private:									//elementos privados: atributos
	...
	void flotar(int i);						//metodo que realiza el movimiento hacia arriba, en busca de la posici�n de inserci�n
	void flotar2(int i);						//metodo que realiza el movimiento hacia arriba, en busca de la posici�n de inserci�n
	bool Lleno(); 							//metodo que determina si monticulo est� lleno
	void criba (int r);						//metodo que modifica posicion de nodo raiz para ubicarlo en la posici�n requerida para cumplir condicion de monticulo. Llamado desde eliminar minimo
...
public:										//metodos publicos
	...
	void Insertar(Clave e); 				//a�ade al mont�culo
	void Insertar2(Clave e); 				//a�ade al mont�culo
	void ordenar(Clave val[], int n); 		// el vector tiene n datos en las posiciones 0,1,�n-1
	Clave BuscarMinimo(); 					//retorna el elemento menor del mont�culo
	Clave EliminarMinimo(); 				//elimina el primer elemento
	bool Esvacio(); 						// est� vac�o
};
...
void Monticulo::flotar(int i){					//realiza el movimiento hacia arriba, en busca de la posici�n de inserci�n
	...
	while ((i > 0) && (val[getPosPadre(i)] > aux)){	//mientras haya nodos padre
		...
		i = getPosPadre(i); 					//sube un nivel en el �rbol
	}
	val[i] = aux; 								//sit�a el valor en la posici�n adecuada
}
...
void Monticulo::Insertar(Clave v){				//inserta nuevo dato en monticulo
	...
	if (!Lleno()) {								//monticulo no lleno => insertar
		val[getNumDatosAct()] = v;				//a�ade valor nuevo en vector de valores
...
void Monticulo::flotar2(int i){					//realiza el movimiento hacia arriba, en busca de la posici�n de inserci�n
	Clave aux = val[i]; 						//se almacena el valor a flotar
	while ((i > 0) && (val[getPosPadre(i)] < aux)){	//mientras haya nodos padre
		val[i] = val[getPosPadre(i)];			//valor del nodo padre baja al espacio encontrado
		i = getPosPadre(i); 					//sube un nivel en el �rbol
...
void Monticulo::Insertar2(Clave v){				//inserta nuevo dato en monticulo

	if (!Lleno()) {								//monticulo no lleno => insertar
		val[getNumDatosAct()] = v;				//a�ade valor nuevo en vector de valores
		flotar2(getNumDatosAct());				//desplaza elementos para conservar relacion entre elmentos
		setNumDatosAct(getNumDatosAct()+1);		//incrementa contador de numero de datos actual
	}
	else {
		cout << endl << "ERROR....Montículo lleno...";	//genera condicion de error
...
void Monticulo::ordenar(Clave val[], int n){ 			// el vector tiene n datos en las posiciones 0,1,�n-1
	...
	for (j = (n - 1) / 2; j >= 0; j--){					//intercambiar los extremos, decrementar en 1 la posici�n del extremo superior y reconstruir el mont�culo del nuevo vector
		criba1(val, j, n-1 );
	}
...
Clave Monticulo:: BuscarMinimo(){				//determina el valor minimo del monticulo
	...
	cout << endl << "ERROR....Mont�culo vacio...";	//genera condicion de error
...
void Monticulo::criba(int r){						//modifica posicion de nodo raiz para ubicarlo en la posici�n requerida para cumplir condicion de monticulo
	...
	while ((hm <= getNumDatosAct()-1) && !esMont){ 	// determina el �ndice del hijo menor
		if (hm < (getNumDatosAct() - 1)){ 			//�nico descendiente
		...
		if (val[hm] < val[r]){						//compara raiz con el menor de los hijos
			...
			r = hm;									//en raiz pone hijo menor continua rama de claves m�nimas
...
Clave Monticulo::EliminarMinimo(){				//elimina el primer nodo del monticulo y en su lugar pone el ultimo elemento. Con Criba implementa condicion de monticulo
	if (!Esvacio()) {
		...
		setNumDatosAct(getNumDatosAct()-1);		//disminuye el tama�o del monticulo
...
void Monticulo::criba1(Clave val[], int r, int ult){
	...
	while (hj <= ult && !esMont){ 					//determina el �ndice del hijo mayor
		if (hj < ult ){ 							//�nico descendiente
		...
		if (val[hj] > val[r]){						//compara raiz con el mayor de los hijos
			...
			val[hj] = c;
			r = hj; 								// contin�a por la rama de claves m�nimas
		} else {
